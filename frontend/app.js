...
// generated by `parse_excel.py`. Wait until external assets are ready before
// creating the visualizations so the map is visible prior to adding markers.
start();
...
function ensureAssets() {
  return new Promise((resolve, reject) => {
    let attempts = 0;
    const check = () => {
      if (window.L && window.Cesium) {
        console.log('Leaflet and Cesium scripts loaded');
        resolve();
      } else if (attempts++ < 20) {
        setTimeout(check, 250);
      } else {
        reject(new Error('Leaflet or Cesium failed to load'));
      }
    };
    check();
  });
...
function initCesium() {
  ...
  const imageryReady = viewer.imageryLayers.get(0).imageryProvider.readyPromise;

  return imageryReady.then(() => {
    console.log('Cesium imagery loaded');
    addCesiumEntities();
  });
}

function addCesiumEntities() {
  const pinUrl = 'https://unpkg.com/leaflet@1.7.1/dist/images/marker-icon.png';
